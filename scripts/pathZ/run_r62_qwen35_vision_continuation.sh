...
LOG="$LOGDIR/r62_train_qwen35_r57b_plus_genesis_vision_e2.log"

mkdir -p "$LOGDIR"
cd "$REPO"

{
  echo "[$(date)] === r62 Qwen3.5 r57b + Genesis vision continuation start ==="
  echo "base_adapter=outputs/r57b/checkpoint-final"
  echo "train_jsonl=data/pathZ/genesis_vision_rebuild/train.expanded.jsonl"
  echo "output_dir=outputs/r62_qwen35_r57b_plus_genesis_vision_e2"
} | tee "$LOG"
...
uv run python scripts/pathZ/train_smoke.py \
  ...
  --output-dir outputs/r62_qwen35_r57b_plus_genesis_vision_e2 \
  --epochs 2 \
  --lr 5e-5 \
  --batch-size 1 \
  --grad-accum 4 \
...
echo "[$(date)] === r62 training done ===" | tee -a "$LOG"
